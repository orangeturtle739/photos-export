...
for master in bar(iter(c.fetchone, None)):
    ...
    for version in iter(vc.fetchone, None):
        ...
        if is_master:
            ...
            for foo in edited_path:
                uuid = hashlib.sha256(open(foo, 'rb').read()).hexdigest()
    ...
    uuid = hashlib.sha256(open(master_path, 'rb').read()).hexdigest()
    if uuid in uuids:
    ...
    for edit_info in edited_paths:
        ...
        with open(os.path.join(output_dir, '%s.json' % edit_info['euuid']), 'w') as log_file:
            print(
                json.dumps(
                    dict(
                        dict(
